...
cm-pill-contains-none = Таблетка не содержит реагентов.
cm-pill-contents-unknown = Вы не знаете, что в нем находится.
cm-pill-contents-none = { "" }
cm-pill-contents-bicaridine = Пилюля "Бикаридин". Лечит тупой урон.
...
cm-pill-contents-inaprovaline = Таблетка "Инапровалин". Используется для стабилизации состояния пациентов.
cm-pill-contents-kelotane = Таблетки "Келотан". Используется для лечения ожогов.
cm-pill-contents-tricordrazine = Таблетка "Трикордразин". Слабое лекарство общего назначения для лечения повреждений.
