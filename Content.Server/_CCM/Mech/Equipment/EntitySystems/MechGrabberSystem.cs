...
namespace Content.Server._CCM.Mech.Equipment.EntitySystems;

/// <summary>
/// Handles <see cref="CCMMechGrabberComponent"/> and all related UI logic
/// </summary>
public sealed class MechGrabberSystem : EntitySystem
{
    ...
    {
        SubscribeLocalEvent<CCMMechGrabberComponent, MechEquipmentUiMessageRelayEvent>(OnGrabberMessage);
        SubscribeLocalEvent<CCMMechGrabberComponent, ComponentStartup>(OnStartup);
        SubscribeLocalEvent<CCMMechGrabberComponent, MechEquipmentUiStateReadyEvent>(OnUiStateReady);
        SubscribeLocalEvent<CCMMechGrabberComponent, MechEquipmentRemovedEvent>(OnEquipmentRemoved);
        SubscribeLocalEvent<CCMMechGrabberComponent, AttemptRemoveMechEquipmentEvent>(OnAttemptRemove);

        SubscribeLocalEvent<CCMMechGrabberComponent, AfterInteractEvent>(OnAfterInteract);
        SubscribeLocalEvent<CCMMechGrabberComponent, GrabberDoAfterEvent>(OnMechGrab);
    }

    private void OnGrabberMessage(EntityUid uid, CCMMechGrabberComponent component, MechEquipmentUiMessageRelayEvent args)
    {
        if (args.Message is not MechGrabberEjectMessage msg)
            return;

        if (!TryComp<CCMMechEquipmentComponent>(uid, out var equipmentComponent) ||
    ...
    public void RemoveItem(EntityUid uid, EntityUid mech, EntityUid toRemove, CCMMechGrabberComponent? component = null)
    {
    ...
    private void OnEquipmentRemoved(EntityUid uid, CCMMechGrabberComponent component, ref MechEquipmentRemovedEvent args)
    {
        if (!TryComp<CCMMechEquipmentComponent>(uid, out var equipmentComponent) ||
            equipmentComponent.EquipmentOwner == null)
            return;
    ...
    private void OnAttemptRemove(EntityUid uid, CCMMechGrabberComponent component, ref AttemptRemoveMechEquipmentEvent args)
    {
        args.Cancelled = component.ItemContainer.ContainedEntities.Any();
    }

    private void OnStartup(EntityUid uid, CCMMechGrabberComponent component, ComponentStartup args)
    {
        component.ItemContainer = _container.EnsureContainer<Container>(uid, "item-container");
    }

    private void OnUiStateReady(EntityUid uid, CCMMechGrabberComponent component, MechEquipmentUiStateReadyEvent args)
    {
    ...
    private void OnAfterInteract(EntityUid uid, CCMMechGrabberComponent component, AfterInteractEvent args)
    {
        if (args.Handled)
            return;
    ...
    private void OnMechGrab(EntityUid uid, CCMMechGrabberComponent component, DoAfterEvent args)
    {
        ...
        if (!TryComp<CCMMechEquipmentComponent>(uid, out var equipmentComponent) || equipmentComponent.EquipmentOwner == null)
            return;
        var mech = equipmentComponent.EquipmentOwner.Value;
