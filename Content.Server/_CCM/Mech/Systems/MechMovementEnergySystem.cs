...
public sealed class MechMovementEnergySystem : EntitySystem
{
    ...
    {
        base.Initialize();
        SubscribeLocalEvent<CCMMechComponent, MechMovementDrainToggleEvent>(OnDrainToggle);
    }

    private void OnDrainToggle(EntityUid uid, CCMMechComponent component, ref MechMovementDrainToggleEvent args)
    {
        if (args.Enabled)
    ...
    public override void Update(float frameTime)
    {
        ...
        {
            if (!TryComp<CCMMechComponent>(mechUid, out var mech) || !TryComp<InputMoverComponent>(mechUid, out var mover))
            {
