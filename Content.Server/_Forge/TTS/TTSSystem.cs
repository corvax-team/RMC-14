...
public sealed partial class TTSSystem : EntitySystem
{
    ...
    private async void OnEntitySpoke(EntityUid uid, TTSComponent component, EntitySpokeEvent args)
    {
        ...
        // CM has no language system; whispers carry an obfuscated message, normal speech does not.
        var isWhisper = args.ObfuscatedMessage != null;
        await Handle(uid, args.Message, protoVoice.Speaker, isWhisper, args.ObfuscatedMessage);
    }
        ...
        string? obfuscatedMessage)
    {
        ...
        // For non-whispers, everyone in range hears the clear message.
        if (!isWhisper || obfuscatedMessage is null)
        {
            RaiseNetworkEvent(fullTtsEvent, Filter.Pvs(uid));
            return;
        }
        ...
        foreach (var session in recipients)
        {
            ...
            var getsClearWhisper = distance <= ChatSystem.WhisperClearRange;

            RaiseNetworkEvent(getsClearWhisper ? fullTtsEvent : obfTtsEvent, session);
        }
    }
