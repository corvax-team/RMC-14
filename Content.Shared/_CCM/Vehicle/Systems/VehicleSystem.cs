...
public sealed partial class VehicleSystem : EntitySystem
{
    ...
    public override void Initialize()
    {
        ...
        SubscribeLocalEvent<CCMVehicleComponent, BeforeDamageChangedEvent>(OnBeforeDamageChanged);
        SubscribeLocalEvent<CCMVehicleComponent, UpdateCanMoveEvent>(OnVehicleUpdateCanMove);
        SubscribeLocalEvent<CCMVehicleComponent, ComponentShutdown>(OnVehicleShutdown);
        SubscribeLocalEvent<CCMVehicleComponent, GetAdditionalAccessEvent>(OnVehicleGetAdditionalAccess);

        SubscribeLocalEvent<CCMVehicleOperatorComponent, ComponentShutdown>(OnOperatorShutdown);
    }

    /// <remarks>
    /// We subscribe to BeforeDamageChangedEvent so that we can access the damage value before the container is applied.
    /// </remarks>
    private void OnBeforeDamageChanged(Entity<CCMVehicleComponent> ent, ref BeforeDamageChangedEvent args)
    {
    ...
    private void OnVehicleUpdateCanMove(Entity<CCMVehicleComponent> ent, ref UpdateCanMoveEvent args)
    {
    ...
    private void OnVehicleShutdown(Entity<CCMVehicleComponent> ent, ref ComponentShutdown args)
    {
        TryRemoveOperator(ent);
    }

    private void OnVehicleGetAdditionalAccess(Entity<CCMVehicleComponent> ent, ref GetAdditionalAccessEvent args)
    {
    ...
    private void OnOperatorShutdown(Entity<CCMVehicleOperatorComponent> ent, ref ComponentShutdown args)
    {
    ...
    public bool TrySetOperator(Entity<CCMVehicleComponent> entity, EntityUid? uid, bool removeExisting = true)
    {
        ...
        if (TryComp<CCMVehicleOperatorComponent>(uid, out var eOperator))
        ...
        if (entity.Comp.Operator is { } currentOperator && TryComp<CCMVehicleOperatorComponent>(currentOperator, out var currentOperatorComponent))
        {
            ...
            RemCompDeferred<CCMVehicleOperatorComponent>(currentOperator);
        ...
        if (uid != null)
        {
            ...
            var vehicleOperator = AddComp<CCMVehicleOperatorComponent>(uid.Value);
    ...
    [PublicAPI]
    public bool TryRemoveOperator(Entity<CCMVehicleComponent> entity)
    {
    ...
    [PublicAPI]
    public bool TryRemoveOperator(Entity<CCMVehicleOperatorComponent?> operatorEntity)
    {
        if (!Resolve(operatorEntity, ref operatorEntity.Comp, false))
            return true;

        if (!TryComp<CCMVehicleComponent>(operatorEntity.Comp.Vehicle, out var vehicle))
            return true;
    ...
    [PublicAPI]
    public bool TryGetOperator(Entity<CCMVehicleComponent?> entity, [NotNullWhen(true)] out Entity<CCMVehicleOperatorComponent>? operatorEnt)
    {
        operatorEnt = null;
        ...
        if (!TryComp<CCMVehicleOperatorComponent>(operatorUid, out var operatorComponent))
            return false;
    ...
    /// </summary>
    public EntityUid? GetOperatorOrNull(Entity<CCMVehicleComponent?> entity)
    {
    ...
    /// </summary>
    [PublicAPI]
    public bool HasOperator(Entity<CCMVehicleComponent?> entity)
    {
    ...
    /// </summary>
    public bool CanOperate(Entity<CCMVehicleComponent?> entity, EntityUid uid)
    {
        if (!Resolve(entity, ref entity.Comp))
            return false;
    ...
    /// </summary>
    public void RefreshCanRun(Entity<CCMVehicleComponent?> entity)
    {
    ...
    private void UpdateAppearance(Entity<CCMVehicleComponent> entity)
    {
