...
namespace Content.Client._CCM.UserInterface.Controls
{
    ...
    public partial class DefaultCMWindow : BaseWindow
    {
        ...
        private int _closeAnimationGeneration;
        private const string WindowOpenAnimationKey = "ccm-window-open";
        private const string WindowCloseAnimationKey = "ccm-window-close";
        ...
        {
            _closingAnimated = false;
            StopAnimation(WindowCloseAnimationKey);
            StopAnimation(WindowOpenAnimationKey);

            Modulate = Color.White.WithAlpha(0.88f);

            if (!IsOpen)
                OpenCentered();

            Visible = true;
            PlayAnimation(BuildFadeAnimation(Color.White.WithAlpha(0.88f), Color.White), WindowOpenAnimationKey);
        }

        public void CloseAnimated()
        {
            if (_skipCloseAnimation || Disposed || !IsOpen)
            {
                Close();
                return;
            }
            ...
            _closeAnimationGeneration++;
            var generation = _closeAnimationGeneration;
            StopAnimation(WindowOpenAnimationKey);
            StopAnimation(WindowCloseAnimationKey);
            PlayAnimation(BuildFadeAnimation(Modulate, Color.Transparent), WindowCloseAnimationKey);
            Timer.Spawn(TimeSpan.FromSeconds(0.18f), () =>
            {
                if (Disposed || !_closingAnimated || generation != _closeAnimationGeneration)
                    return;

                FinalizeAnimatedClose();
            });
        }

        private void OnAnimationCompleted(string key)
        {
            if (key != WindowCloseAnimationKey || !_closingAnimated)
                return;

            FinalizeAnimatedClose();
        }

        private void FinalizeAnimatedClose()
        {
            _closingAnimated = false;
            Modulate = Color.White;
            _skipCloseAnimation = true;
            Close();
            _skipCloseAnimation = false;
        }

        private static Animation BuildFadeAnimation(Color from, Color to)
        {
