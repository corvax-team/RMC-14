...
using Content.Shared._RMC14.Xenonids.Slam;
...
namespace Content.Client._RMC14.Xenonids.Slam;

public sealed class XenoTailSlamSystem : SharedXenoTailSlamSystem
{
...
#if DEBUG
        _console.RegisterCommand("toggleshowtailslam", ToggleShowTailSlam);
#endif
    }
...
#if DEBUG
        _console.UnregisterCommand("toggleshowtailslam");
#endif
    }

    private void ToggleShowTailSlam(IConsoleShell shell, string s, string[] strings)
    {
        _showTailAttack = !_showTailAttack;

        if (_showTailAttack)
        {
            var overlay = new TailSlamOverlay();
            _overlays.AddOverlay(overlay);
            shell.WriteLine("Enabled showing tail attack hitboxes");
        }
        else
        {
            _overlays.RemoveOverlay<TailSlamOverlay>();
            shell.WriteLine("Disabled showing tail attack hitboxes");
        }
    }

    protected override void DoLunge(Entity<XenoTailSlamComponent, TransformComponent> user, Vector2 localPos, EntProtoId animationId)
    {
    ...
    public override void FrameUpdate(float frameTime)
    {
        ...
        if (_overlays.TryGetOverlay(out TailSlamOverlay? overlay))
        {
            overlay.Last = LastTailAttack;
        }
    }

    private sealed class TailSlamOverlay : Overlay
    {
